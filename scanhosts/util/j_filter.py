...
class FilterRules():
        ...
        for key_obj in key_obj_list:
            ...
            if sn_key and sn_key != u'' and sn_key != 'NotSpecified':
                if sn_key not in self.key_dic_map.keys():
                    self.key_dic_map[sn_key] = [key_obj.ip, key_obj.ssh_port, key_obj.ssh_user, key_obj.ssh_passwd,
                                                key_obj.ssh_rsa, key_obj.rsa_pass, key_obj.system_ver,
                                                key_obj.hostname, key_obj.ssh_status, key_obj.ssh_type,
                                                key_obj.mac_address, sn_key, key_obj.mathine_type]
                else:
                    self.key_dic_map[sn_key][0] = self.key_dic_map[sn_key][0] + "," + key_obj.ip
            elif mac_addr:
                if mac_addr not in self.key_dic_map.keys():
                    self.key_dic_map[mac_addr] = [key_obj.ip, key_obj.ssh_port, key_obj.ssh_user, key_obj.ssh_passwd,
                                                  key_obj.ssh_rsa, key_obj.rsa_pass, key_obj.system_ver,
                                                  key_obj.hostname, key_obj.ssh_status, key_obj.ssh_type,
                                                  key_obj.mac_address, key_obj.sn, key_obj.mathine_type]
                else:
                    self.key_dic_map[mac_addr][0] = self.key_dic_map[mac_addr][0] + "," + key_obj.ip
            else:
                print("error:no sn and no mac:%s" % key_obj.id)
    ...
    def insert_db_conn_info(self):
        ...
        for item, item_val in self.key_dic_map.items():
            ConnectionInfo.objects.create(id=id, ssh_username=item_val[2], ssh_userpasswd=item_val[3],
                                          ssh_hostip=item_val[0], ssh_host_port=item_val[1], ssh_rsa=item_val[4],
                                          rsa_pass=item_val[5],
    ...
    def filter_type(self):
        ...
        for item, item_val in self.key_dic_map.items():
            ...
            if re.search("KVM", item_val[-1]) or re.search("QEMU", item_val[-1]):
                VirtualServerInfo.objects.create(server_ip=item_val[0], server_type=item_val[-1],
                                                 system_ver=item_val[6], sys_hostname=item_val[7], mac=item_val[-3],
                                                 sn=item_val[-2], vir_phy_id=1,
                                                 conn_vir_id=int(self.key_sn_relate[mac_addr]))
            elif re.search("VMware", item_val[-1]):
                mac_addr = self.return_key_mac(item_val[-3])
                VirtualServerInfo.objects.create(server_ip=item_val[0], server_type=item_val[-1],
                                                 system_ver=item_val[6], sys_hostname=item_val[7], mac=item_val[-3],
                                                 sn=item_val[-2], vir_phy_id=1,
                                                 conn_vir_id=int(self.key_sn_relate[sn_key]))
                ...
                if sn_key and sn_key != 'NotSpecified':
                    PhysicalServerInfo.objects.create(server_ip=item_val[0], machine_brand=item_val[-1],
                                                      system_ver=item_val[6], sys_hostname=item_val[7],
                                                      mac=item_val[-3], sn=item_val[-2], ser_cabin_id=1,
                                                      conn_phy_id=int(self.key_sn_relate[sn_key]))
                else:
                    PhysicalServerInfo.objects.create(server_ip=item_val[0], machine_brand=item_val[-1],
                                                      system_ver=item_val[6], sys_hostname=item_val[7],
                                                      mac=item_val[-3], sn=item_val[-2], ser_cabin_id=1,
                                                      conn_phy_id=int(self.key_sn_relate[mac_addr]))
