            ...
            for password in password_list:
                if port not in self.can_login_lst.keys():
                    ...
                    if res["status"] == "success":
                        ...
                        print("ssh login and exec command:%s", res)
                        logger.info("ssh login and exec command:%s", res)
                        self.can_login_lst[port] = (
                        ip, password, 'root', system_info, sys_hostname, sys_mac, sys_sn, machine_type)
                    elif res["status"] == "failed" and re.search(r"reading SSH protocol banner", res["res"]):
                        # print "res res..........................",res['res']
        ...
        for ip, port in sship_list.items():
            ...
            if ip not in self.can_key_login_lst.keys():
                ...
                if res["status"] == "success":
                    ...
                    port, keyfile, "root", "", 1, system_info, sys_hostname, sys_mac, sys_sn, machine_type)
                if res["status"] == "failed":
                    ...
                    if res["status"] == "success":
                        ...
                        self.can_key_login_lst[ip] = (
                        port, keyfile, "root", "", 2, system_info, sys_hostname, sys_mac, sys_sn, machine_type)
                    else:
    ...
    for ip, login_info in dev_ips.items():
        ...
        if res["status"] == "success" and res["level"] == 2:
            ...
            if dev_obj:
                ...
                if not con_obj:
                    NetConnectionInfo.objects.create(tel_username=con_user, tel_userpasswd=crpt_pass,
                                                     tel_enpasswd=en_crpt_pass,
                                                     tel_host_port='21', tel_hostip=ip, sn_key=dev_sn_key,
                                                     dev_info_id=dev_id, tel_status=1, tel_type=int(res["level"]))
